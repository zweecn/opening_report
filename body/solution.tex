...
对于候选服务，是以服务编号和~QoS~组成的二元组，如式(~\ref{cs}~)所示。其QoS是以执行时间~$t$~、价格~$p$~，可靠性~$r$~组成的三元组，如式(~\ref{qos}~)所示。
...
\subsubsection{决策收益} \label{reward_section}
...
业务层面的不确定性采用海运物流业务作为示例，其多方之间的工作流程如图~\ref{bp_example}~所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.95\textwidth]{bp_example}
    \caption{海运物流工作流}\label{bp_example}
    \vspace{-1em}
\end{figure}

如图~\ref{bp_example}~所示的工作流图实际上就是一个有向图，将其简化转换之后如图~\ref{bp_ws_process}~所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width = 0.7\textwidth]{bp_ws_process}
    \caption{海运物流业务DAG图}\label{bp_ws_process}
    \vspace{-1em}
\end{figure}
...
业务层面执行比软件层面有更多复杂的不确定性事件，其表示方式如式(\ref{event})所示。其不确定性事件的类型除了~\ref{uc_event_section}~节所介绍的之外，还增加两类事件：
...
其不确定性事件在不同的活动中是不一样的，因此枚举所有活动中所有可能的不确定性事件如表~\ref{ocean_shipping_uc}~所示。

% Table generated by Excel2LaTeX from sheet 'Sheet8'
%\begin{table}[htbp]
%    \caption{海运物流中的不确定性}
%    \vspace{-0.5em}\label{ocean_shipping_uc}\centering\zihao{5}
%    \begin{tabular}{cccc}
%         \toprule
...
(5)~报酬函数~$Reward$~：在~\ref{reward_section}~节中给出。
